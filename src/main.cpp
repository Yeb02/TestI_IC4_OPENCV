...
int main()
{
	...
	Optimizer::IMAGE_SEQ imSeq = Optimizer::IMAGE_SEQ::GRAVIERE;

	std::vector<cv::Mat> dstBayerImgs;
	std::vector<cv::Mat> dstRGBImgs;

	bool showSequence = false;
	optimizer.LoadAndPreProcess(imSeq, showSequence, dstBayerImgs, dstRGBImgs);

	//auto stack = optimizer.WeightedTotalCombination(dstBayerImgs, dstRGBImgs);

	auto stack = optimizer.FullFrameSequentialMatcher(dstBayerImgs, dstRGBImgs);


	//auto pointVecs = optimizer.OptFlow(dstRGBImgs);
